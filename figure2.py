...
print( params["Hexokinase-phosphofructokinase n"]["kx"] )
print( params["Hexokinase-phosphofructokinase g"]["kx"] )
"""
agents = []
...
ynotnans = np.logical_not( np.isnan( y[14, :] ) )
y = y[:, ynotnans]
t = t[ynotnans]
...
for i in range(runtimes):
    tminstim = 10 + i*one_simulation_time
    tmaxstim = tminstim + 20
    ext_g_input[(t>=tminstim)&(t<=tmaxstim)] = params["stimulation"]["ge"] * np.exp(-(t[(t>=tminstim)&(t<=tmaxstim)] - tminstim)/ params["stimulation"]["tau"])


broken_barh_x = [(10 + (i*one_simulation_time), 20) for i in range(runtimes)]
...
axes[0].broken_barh(broken_barh_x, (- 2, 1000), color="gray", alpha=0.2)
...
for i, idx in enumerate(indxes):
    ...
    axes[i].broken_barh( broken_barh_x, (ymin-2, 1000), color="gray", alpha=0.2 )
    axes[i].set_ylabel(units[idx])

axes[i].set_xlabel("Время, сек")

fig1.savefig("./figures/fig4.png", dpi=300)
...
for i, idx in enumerate(indxes):
    ...
    axes[i, j].broken_barh( broken_barh_x, (ymin-2, 1000), color="gray", alpha=0.2 )
...
fig2.savefig("./figures/fig5.png", dpi=300)
