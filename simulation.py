...
class Simulator():
    ...
    def __init__(self, enzymes, metabolites):
        ...
        self.jacobian = np.zeros([len(metabolites), len(metabolites)], dtype=np.float)
    ...
    def run_model(self, t, y):
        ...
        for enzyme in self.enzymes:
            dydt = enzyme.update(y, dydt, self.jacobian)
        dydt[65] *= 10e-15 # !!!!!!!!
        return dydt
...
sol = solve_ivp(simulalor.run_model, [0, 100], y0, method="LSODA", min_step=0.001, rtol=0.001,   atol=1e-6) #, jac=simulalor.get_jacobian min_step=0.001,  LSODA
